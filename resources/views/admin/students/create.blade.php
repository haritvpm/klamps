    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.students.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('roll_number') ? 'is-invalid' : '' }}" type="text"  pattern="^\S+$" name="roll_number" id="roll_number" value="{{ old('roll_number', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('fee_paid') ? 'is-invalid' : '' }}" name="fee_paid" id="fee_paid">
                    ...
                    @foreach(App\Models\Student::FEE_PAID_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('fee_paid', 'n') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('centre') ? 'is-invalid' : '' }}" name="centre" id="centre" required>
                    ...
                    @foreach(App\Models\Student::CENTRE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('centre', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
